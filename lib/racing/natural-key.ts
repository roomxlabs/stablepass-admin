...
// junk that the create route's original falsy check rejected outright, so the shared helper must be
// able to reject it too — otherwise extracting the invariant would LOOSEN create. `race_number`
// legitimately arrives as a JSON number, so it stays type-tolerant and is range-checked by its caller.
...
//
// `stringOnly` additionally rejects non-string input. Create passes it for venue/race_date to keep
// the strictness its old falsy check had. PATCH deliberately does NOT pass it: ENG-324 shipped the
// type-tolerant form and changing that is out of this ticket's scope (follow-up noted in .rx/gotchas.md).
